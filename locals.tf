locals {
  ...
  image_metadata_boot_volumes = [for volume in local.image_metadata_body.volumes: merge({size=40}, volume) if volume.boot]
  image_metadata_local_volumes = [for volume in local.image_metadata_body.volumes: merge({size=40}, volume) if volume.boot == false]
